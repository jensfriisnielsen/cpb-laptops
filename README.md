...
[`hosts/koderup27/default.nix`](hosts/koderup27/default.nix) imports [`modules/server`](modules/server). That collection keeps the machine awake, trusts the Netbird interface `wt0`, serves `koderup.dk` from dnsmasq on the Netbird address, and runs Traefik and Homepage with Podman Compose. Another host can import `modules/server` the same way.

Certificates are not generated by Nix. From the devshell:

```sh
just certs
```

That runs [`scripts/koderup-certs.sh`](scripts/koderup-certs.sh). mkcert writes these under `secrets/mkcert` (git-crypt):

- `rootCA.pem` and `rootCA-key.pem`, the CA
- `koderup.crt` and `koderup.key`, the certificate for `koderup.dk` and `*.koderup.dk`
- `koderup-fullchain.crt`, the leaf followed by the CA

Only `koderup.key` is sops-encrypted, to `secrets/koderup.yaml`. Commit `secrets/mkcert` and `secrets/koderup.yaml`.

After the next rebuild, every laptop trusts `rootCA.pem` in Firefox, Chromium, and Brave. LibreWolf is unmanaged. Traefik serves the full chain. `https://home.koderup.dk` and `https://koderup.dk` are Homepage.

Split DNS is set in the Netbird dashboard, not in this repo. DNS → Nameservers → Add Nameserver → Custom DNS. Use koderup27’s Netbird IPv4 from `netbird status`, port 5353, distribute it to all peers, and set the match domain to `koderup.dk` (subdomains are included; do not write `*.koderup.dk`). Leave it non-primary so other names keep using the existing resolver. Port 53 on that address is Netbird’s own DNS listener.

On a laptop, `resolvectl query home.koderup.dk` should return koderup27’s Netbird address.
